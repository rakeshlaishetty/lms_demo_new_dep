...
router.get('/getFeeStructure', async (req, res) => {
    const data = await FeeStructure.find({schoolId: req.query.schoolId});
    res.status(200).send(data);
})

router.get('/getFeesById', async (req, res) => {
    const stud = await Student.findOne({_id: req.query.studentId});
    console.log(stud)
    res.status(200).send({tuition: stud.fees.tuition, transport: stud.fees.transport, admission: stud.fees.admission, sports: stud.fees.sports, payment: stud.paymentType});
})

router.get('/getFeeStructureClass', async (req, res) => {
    const data = await FeeStructure.findOne({schoolId: req.query.schoolId, class: req.query.class});
    res.status(200).send(data);
})

router.post('/editFeeStructure', async (req, res) => {
    if(isNaN(req.body.tuition) || isNaN(req.body.transport) || isNaN(req.body.admission) || isNaN(req.body.sports)){
        res.status(400).send({msg: 'Please enter numeric values for all fields!'});
        return;
    }
    await FeeStructure.updateOne({class: req.body.class}, {$set: {
    ...
    .catch((err) => res.status(500).send({msg: 'Something went wrong, please try again.'}))
})
...
router.get('/getRecentFeePayments', async (req, res) => {
    let data = await FeeTransaction.find({schoolId: mongoose.Types.ObjectId(req.query.schoolId)}).limit(10).sort({$natural:-1}).populate('studentId');
    res.status(200).send(data);
})
